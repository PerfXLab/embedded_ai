...
简评：NVDIA发布TensorRT 3（支持众多深度学习框架），基于Tesla V100的HGX-1加速器，Volta架构的新一代终端芯片Xavier。
...
简评：作者提出模型架构和超参数的选择没有一个指导，所以将连续型的超参数如学习率，以及离散型变量如激活函数的类型等作为参数，通过传统的遗传算法来寻找最佳模型结构和超参数。在7个数据集上的实验上，三个达到当前最好的结果。
